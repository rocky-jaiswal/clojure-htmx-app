...
(defn lists-page [{:keys [csrf-token identity lists]}]
  (layout/base
    {:title "My Lists" :csrf-token csrf-token :identity identity}
    [:div {:class "space-y-6"}
     [:h1 {:class "text-2xl font-bold"} "My Lists"]
     [:form {:method "post" :action "/lists" :class "flex gap-2"}
...
(defn list-detail-page [{:keys [csrf-token identity list items]}]
  (let [list-id (:todo_lists/id list)]
    (layout/base
      ...
      [:div {:class "space-y-6"}
       ...
       [:form {:method "post" :action (str "/lists/" list-id "/items") :class "flex gap-2"}
        [:input {:type "hidden" :name "__anti-forgery-token" :value csrf-token}]
